...
type AnimationComponent = React.ComponentType<{ className?: string }>;
...
export function AnimationShowcase({ 
  ...
  const [selectedAnimationId, setSelectedAnimationId] = useState<number | null>(
  ...
  // Group animations by category
  const animationsByCategory = useMemo(() => groupByCategory(animations), [animations]);
  ...
  const AnimationComponent = useMemo(() => {
    ...
    return function AnimationWrapper() {
      ...
      const props = {
        ...
        ...(selectedAnimation.componentProps || {})
      };
...
// ${error instanceof Error ? error.message : 'Unknown error'}`);
  ...
  // If no animations are provided, show a message
  if (animations.length === 0) {
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-gray-500">No animations available.</p>
      </div>
    );
  }
