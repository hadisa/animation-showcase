...
export default function ShakeCard({
  ...
  const rarityColors = {
    ...
    epic: { glow: "#ffaa00", border: "#ffaa00", accent: "#ffaa00" },
  ...
  return (
    <div
      className="relative w-[340px] h-[480px] mx-auto my-10"
      ...
      >
        ...
        >
          ...
          <div className="particles absolute inset-0 overflow-hidden pointer-events-none">
            {[...Array(15)].map((_, i) => (
              <div
                key={i}
                className="particle absolute w-1 h-1 rounded-full bg-white opacity-70"
                style={{
                  top: `${Math.random() * 100}%`,
          ...
          <div
            className="card-frame w-full h-full animate-card-idle"
            style={{
              boxShadow: isHovered
          ...
          >
            ...
            <div
              className="absolute top-0 left-0 w-0 h-0 border-t-[60px] border-l-[60px] border-t-transparent border-l-transparent"
              style={{ borderRight: `60px solid ${currentRarity.accent}20` }}
            ></div>

            {/* Top Hero Banner */}
            <div
              className="card-banner card-banner-top absolute left-1/2 -translate-x-1/2 top-4 animate-banner-shine group-hover:translate-y-[-2px] group-hover:scale-[1.02]"
              style={{
                background: `linear-gradient(to bottom, ${currentRarity.border}, ${currentRarity.glow})`,
              }}
            >
              <span className="relative z-10 text-white font-bold text-sm tracking-wide drop-shadow-md">
                {heroType}
              </span>
            </div>
            ...
            <div
              className="absolute top-[50px] left-1/2 -translate-x-1/2 w-full h-[70%] rounded-lg overflow-hidden border-[3px] shadow-inner-image group-hover:border-[#ffd700] transition-all duration-500"
            ...
            <div
              className="card-circle card-circle-bottom-left absolute bottom-5 left-5 flex items-center justify-center group-hover:scale-110 transition-transform duration-300"
              style={{
          ...
          <style jsx global>{`
            ...
            .text-shadow-heavy {
              text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.8),
                -1px -1px 2px rgba(0, 0, 0, 0.8), 0 0 5px rgba(0, 0, 0, 0.8);
            }
            ...
            .card-frame::before {
              ...
              background: radial-gradient(
                ...
                linear-gradient(
                  45deg,
                  rgba(139, 128, 83, 0.2) 25%,
                  transparent 25%,
                  transparent 50%,
                  rgba(139, 128, 83, 0.2) 50%,
                  rgba(139, 128, 83, 0.2) 75%,
                  transparent 75%,
                  transparent
                );
        ...
        <Card className="absolute w-full h-full backface-hidden rotate-y-180 bg-gradient-to-br from-gray-800 to-gray-900 border-2 border-gold rounded-[16px] flex items-center justify-center p-6">
          <div className="text-center text-white">
            ...
            <p className="mb-6">
              Additional information about this card appears here when flipped.
            </p>
            <button
              onClick={() => setIsFlipped(false)}
              className="px-4 py-2 bg-gold text-gray-900 font-bold rounded"
            >
              Flip Back
            </button>
          </div>
